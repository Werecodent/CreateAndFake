...
public sealed class TypeInfoCreateHint : CreateHint
{
    ...
    private static readonly FrozenDictionary<Type, Func<RandomizerChainer, object>> _Gens
        = new Dictionary<Type, Func<RandomizerChainer, object>>()
        {
            ...
            { typeof(ConstructorInfo), rand => FindTypeInfo(rand,
                t => t.GetConstructors().Where(c => c.IsPublic)) },
            { typeof(PropertyInfo), rand => FindTypeInfo(rand,
                t => t.GetProperties()) },
            { typeof(MethodInfo), rand => FindTypeInfo(rand,
                t => t.GetMethods().Where(m => m.IsPublic).Where(m => !m.ReturnType.Inherits(typeof(ValueTuple<,>)))) },
            { typeof(FieldInfo), rand => FindTypeInfo(rand,
                t => t.GetFields().Where(f => f.IsPublic)) },
            { typeof(ParameterInfo), rand => FindTypeInfo(rand,
                t => t.GetMethods().SelectMany(m => m.GetParameters())) },
            { typeof(MethodBase), rand => FindTypeInfo(rand,
                t => t.GetConstructors().Cast<MethodBase>().Concat(
                    t.GetMethods().Where(m => !m.ReturnType.Inherits(typeof(ValueTuple<,>)))).Where(m => m.IsPublic)) },
        }.ToFrozenDictionary();
    ...
    private static T FindTypeInfo<T>(RandomizerChainer randomizer, Func<Type, IEnumerable<T>> grabber)
    {
        T[] result;
        do
        {
