...
public static class ExtractPriorityTests
{
    ...
    {
        ((int)ComparePriority.Highest).Assert().Is(Enum.GetValues<ComparePriority>().Length - 2);
    }

    [Fact]
    internal static void Values_AllPositive()
    {
        Enum.GetValues<ComparePriority>()
            .Except([ComparePriority.Disabled])
            .Cast<int>()
            .Where(v => v < 0)
            .Assert()
            .IsEmpty();
    ...
    internal static void Values_Unique()
    {
        Enum.GetValues<ComparePriority>()
            ...
            .ToHashSet()
            .Assert()
            .HasCount(Enum.GetValues<ComparePriority>().Length);
    }
}
